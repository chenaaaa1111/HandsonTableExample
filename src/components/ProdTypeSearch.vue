...
<mu-paper :z-depth="1">
    <mu-data-table :loading="true" :columns="columns" :sort.sync="sort" @sort-change="handleSortChange" :data="list.slice(0, 6)">
      <template slot-scope="scope">
        ...
        <td class="is-right">{{scope.row.flat}}</td>
        <td class="is-right">{{scope.row.carbs}}</td>
        <td class="is-right">{{scope.row.protein}}</td>
        <td class="is-right">
            <mu-button flat color="primary">引用</mu-button>
  <mu-button flat color="secondary">复制</mu-button>
        </td>
      </template>
    </mu-data-table>
  </mu-paper>
...
export default {
  data() {
    return {
      ...
      columns: [
        { title: "零件号 (含版本)", width: 200, name: "name" },
        {
          title: "零件号",
          name: "calories",
          width: 126,
